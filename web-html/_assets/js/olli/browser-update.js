...
this.op.url= op.url||"http://whatbrowser.org";
this.op.pageurl = op.pageurl || window.location.hostname || "unknown";
this.op.newwindow=op.newwindow||true;
this.op.oldbrowser = op.oldbrowser||false;
...
function getBrowser() {
    ...
    if (/bot|googlebot|facebook|slurp|adsbot|bingbot|google web preview/i.test(ua)) return null;
    ...
    var name = names[n]+" "+v;
    if (document.documentMode !== undefined && v!=document.documentMode)
        name = names[n]+" "+document.documentMode+"*";
    return {n:n,v:v,t:name};
}

this.op.browser=getBrowser();
if (this.op.browser === null) /*its a bot*/
    return;
/*
if (!this.op.test && (!this.op.browser || !this.op.browser.n || this.op.browser.n=="x" || document.cookie.indexOf("browserupdateorg=pause")>-1 || this.op.browser.v>this.op.vs[this.op.browser.n]))
    return;
*/
if (!this.op.test && (!this.op.browser || !this.op.oldbrowser || document.cookie.indexOf("browserupdateorg=pause")>-1))
    return;
/*
...
if (ll=="de")
    ...
    t='Selaimesi (%s) on <b>vanhentunu</b>. Siinä on tunnettuja <b>tietoturvaongelmia</b> eikä se välttämättä tue<b>kaikkia ominaisuuksia</b> tällä tai muilla sivustoilla. <a%s>Lue lisää siitä kuinka päivität selaimesi</a>';
if (op.text)
    t = op.text;
if (op["text_"+ll])
    t = op["text_"+ll];
...
var style = ".buorg {position:absolute;position:fixed;z-index:1;\
width:100%; top:0px; left:0px; \
border-bottom:1px solid #A29330; \
background:#FDF2AB no-repeat 10px center url(img/warning.png);\
...
document.body.className = document.body.className; //Force repaint
